...
for (const job of jobs) {
  const jobitem = queue.create('push_notification_code_2', job).save((err) => {
    if (!err) {
  ...
  jobitem.on('failed', (errorMessage) => {
    console.log(`Notification job ${jobitem.id} failed: ${errorMessage}`);
  });
